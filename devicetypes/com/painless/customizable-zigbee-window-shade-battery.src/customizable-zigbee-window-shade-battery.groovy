...
metadata {
	definition(name: "Customizable ZigBee Window Shade Battery", namespace: "com/painless", author: "SmartThings", ocfDeviceType: "oic.d.blind", mnmn: "SmartThings", vid: "generic-shade-3") {
		capability "Actuator"
		capability "Battery"
	...
	preferences {
		...
		input "reverselevel", "bool", title: "Reverse open/close", description: "Reverse the command for open/close", defaultValue: false, required: false, displayDuringSetup: false
	}
...
def close() {
	...
	setShadeLevel(reverselevel ? 100 : 0)
}

def open() {
	log.info "open()"

	setShadeLevel(reverselevel ? 0 : 100)
}
